...
var createAndSavePerson = function(done) {
    ...
    person.save(function(err, data) {
        if (err) {
            return console.log("Error while creating person:" +err)
        }
        done(null, data);
    });
};
...
var createManyPeople = function(arrayOfPeople, done) {
    Person.create(arrayOfPeople, (err, people) => {
        if (err) return console.log("Error while creating people: " +err);
        done(null, people)
    })
};
...
var findPeopleByName = function(personName, done) {
    Person.find({name: personName}, (err, personFound) => {
        if (err) return console.log("Error while finding personName [" +personName+"]: " +err);
...
var findOneByFood = function(food, done) {
    Person.findOne({favoriteFoods: food}, (err, personFound) => {
        if (err) return console.log("Error while finding food [" +food+"]: " +err);
        done(null, personFound);
    })
};
...
var findPersonById = function(personId, done) {
    Person.findById(personId, (err, personFound) => {
        if (err) return console.log("Error while finding by ID [" +personId+"]: " +err);
        done(null, personFound);
    })
};
...
var findEditThenSave = function(personId, done) {
    Person.findById(personId, (err, personFound) => {
        ...
        //updating personFound
        var foodToAdd = "hamburger";
        personFound.favoriteFoods.push(foodToAdd);
        personFound.save((err, updatedPerson) => {
            if (err) return console.log("Error while updating [" +personFound+"]: " +err);
...
var findAndUpdate = function(personName, done) {
    var ageToSet = 20;
    Person.findOneAndUpdate({name: personName}, {age: ageToSet}, {new: true, useFindAndModify: false}, (err, updatedPerson) => {
        if (err) return console.log("Error while finding by personName [" +personName+"]: " +err);
        done(null, updatedPerson);
    })
};
...
// previous update methods. They pass the removed document to the db.
// As usual, use the function argument `personId` as search key.

var removeById = function(personId, done) {
    Person.findByIdAndRemove(personId, {useFindAndModify: false}, (err, removedPerson) => {
        if (err) return console.log("Error while finding by personId [" +personId+"]: " +err);
...
var removeManyPeople = function(done) {
    ...
    Person.remove({name: nameToRemove}, (err, isRemoved) => {
        if (err) return console.log("Error while removing [" +nameToRemove+"]: " +err);
        done(null, isRemoved);
    })
};
...
var queryChain = function(done) {
    ...
    .exec( (err, personFound) => {
        if (err) return console.log("Error while finding person: " +err);
